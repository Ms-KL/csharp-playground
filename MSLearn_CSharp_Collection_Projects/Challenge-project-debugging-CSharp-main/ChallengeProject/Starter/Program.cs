...
bool useTestData = false; // toggle random (False)
...
while (transactions > 0)
{
    ...
    Console.WriteLine($"# of {transactions} remaining");
    Console.WriteLine(TillAmountSummary(cashTill));
    Console.WriteLine($"Expected till value: {registerCheckTillTotal}");
    Console.WriteLine();
}
...
static void MakeChange(int cost, int[] cashTill, int twenties, int tens = 0, int fives = 0, int ones = 0)
{
    ...
    if (changeNeeded < 0)
    {
        ...
        // reset so till doesn't increase due to error
        //amountPaid = 0;
    }
    ...
    // this prevents having to reverse the value being added to the array in the beginning of the function call
